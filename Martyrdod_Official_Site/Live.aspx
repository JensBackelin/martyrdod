<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.Master" AutoEventWireup="true" CodeBehind="Live.aspx.cs" Inherits="Martyrdod_Official_Site.Live" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <div id="liveContent"><%--Tabeller med info om kommande och tidigare spelningar inkl länkar till biljettköp eller evenemangens websidor--%>
        <div id="Shows">
            <table>
                <th colspan="2" style="text-align: left">
                    <h2>UPCOMMING SHOWS</h2>
                </th>
                <tr>
                    <td>April 4 2015</td>
                    <td>Klubb Dead Rhythm - SWE</td>
                    <td><a href="">Info TBA</a></td>
                </tr>
                <tr>
                    <td>May 23 2015</td>
                    <td>Maryland Deathfest - US</td>
                    <td><a href="http://www.marylanddeathfest.com/band/martyrd%C3%B6d" target="_blank">Tickets</a></td>
                </tr>
                <tr>
                    <td>May 29 2015</td>
                    <td>Temples Festival - UK</td>
                    <td><a href="http://www.templesfestival.co.uk/tickets" target="_blank">Tickets</a></td>
                </tr>
                <tr>
                    <td>July 10 2015</td>
                    <td>Obscene Extreme - CZ</td>
                    <td><a href="http://www.obsceneextreme.cz/index.php?option=com_content&view=article&id=132&Itemid=180&lang=en">Tickets</a></td>
                </tr>
            </table>
            <table>
                <th colspan="2" style="text-align: left">
                    <h2>PAST SHOWS</h2>
                </th>
                <tr>
                    <td>Oct 18 2014</td>
                    <td>Bloodshed Fest - NL</td>
                    <td><a href="http://www.bloodshed.nl/" target="_blank">Info</a></td>
                </tr>
                <tr>
                    <td>Sep 13 2014</td>
                    <td>Enemy Of The Sun Fest - Cz</td>
                    <td><a href="http://www.enemyofthesunfest.com/">Info</a></td>
                </tr>
                <tr>
                    <td>Aug 2014</td>
                    <td>Europa Rising Tour w. Converge</td>
                    <td></td>
                </tr>
                <tr>
                    <td>Jul 12 2014</td>
                    <td>Play fast or dont Fest - Cz</td>
                    <td><a href="http://playfast.czechcore.cz/">Info</a></td>
                </tr>
                <tr>
                    <td>Apr 12 2014</td>
                    <td>Truckstop Alaska - SWE</td>
                    <td></td>
                </tr>
                <tr>
                    <td>Jul 27 2013</td>
                    <td>Fluff Fest - CZ</td>
                    <td></td>
                </tr>
                <tr>
                    <td>Feb 2013</td>
                    <td>EU-tour w. Rotten Sound</td>
                    <td></td>
                </tr>
                <tr>
                    <td>Jun 2012</td>
                    <td>US-tour w. Black Breath</td>
                    <td></td>
                </tr>
                <tr>
                    <td>Jul 16 2011</td>
                    <td>Grundbulten - SWE</td>
                    <td></td>
                </tr>
                <tr>
                    <td>Jun 2011</td>
                    <td>US-tour w. No Static</td>
                    <td></td>
                </tr>
                <tr>
                    <td>Jul 15 2010</td>
                    <td>Obscene Extreme - CZ</td>
                    <td></td>
                </tr>
            </table>
        </div>
        <aside id="liveFlyers">
            <a href="http://www.marylanddeathfest.com/" target="_blank">
                <img src="img/MDF.jpg" /></a>
            <a href="http://www.templesfestival.co.uk/" target="_blank">
                <img src="img/Day Poster Fri May 29th.jpg" /></a>
            <a href="http://www.obsceneextreme.cz/index.php?lang=en&Itemid=181" target="_blank">
                <img src="img/poster-2015--en-600.jpg" /></a>
        </aside>
    </div>

</asp:Content>
